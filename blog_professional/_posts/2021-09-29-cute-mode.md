...
For each state, you'd like to have a few actions that your cat can do: `.MakeSound()` and `.PlayWithToy()`.
In `Angel` mode, `Cat` needs to know which toys are available.
In `Angry` mode, `Cat` doesn't care --- `Cat` is not interested.
However, in `Angry` mode `Cat` needs to know the number of times it's been asked to `.MakeSound()` --- after the third time, `Cat` will start making growls instead of meows.

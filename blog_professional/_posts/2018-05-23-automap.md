...
If the output was wrong, you adjust the parameters of the math equation to make its output less wrong next time.
...
Remember that an autoencoder's goal is to take an input and then give that same input back to you.
...
If we really want to make sure that our autoencoder gets good at reconstructing faces from partial observations (like when the crook's wearing a burglar mask), we can take a sharpie and black out parts of the faces on the input side of the card.
...
Then, the code is read by the decoder (another multilayer artificial neural network) to generate the reconstruction seen on the right.
...
We use the encoder portion of the autoencoder to jump into the latent space (i.e., running the image through the encoder and looking at the output) at the point of departure and the destination.
...
With this approach, the dimensionality of the genotype space remains equivalent to the dimensionality of the phenotype space.
...
We can watch the phenotype change in the phenotype space on the right in step with the corresponding genotype change in the genotype space on the left.
...
Importantly, this genotype-phenotype map allows escape from local fitness peaks in the phenotype space to explore other peaks so evolution is less likely to get stuck and stagnate.
...
We showed that bottlenecked and denoising autoencoder genotype-phenotype maps improve evolvability and demonstrated that, in some circumstances, better evolutionary outcomes can be attained using autoencoder genotype-phenotype mappings.
...
Plus, deep learning and autoencoders are an active area of research.
...
When I was poking around for cool graphics, I stumbled upon my new all-time favorite twitter account, [@smilevector](https://twitter.com/smilevector).
It's a bot run by [Tom White](http://projects.drib.net/) that uses autoencoders to animate smiles onto still pictures.
...
Further work with AutoMap, of course, will also investigate potential pitfalls with the technique and, hopefully, test schemes to circumvent them.

...
Of course, in order to live out that dream, I've gotta set myself up with data to play with when the time arrives!
...
If you use an electronic calendar, you've probably got a solid record of your scheduled events --- the weightlifting sessions with Squee, and everything else --- going back a few years... unless you switched providers.
...
The content of the photos themselves probably tell the richest story, but I think that there might be other interesting threads to pull out of my RAW metadata, like what times of day or weeks of the month I tend to shoot on or how the total shutter count of my camera progresses relative to the number of photos I've chosen to keep.
...
I use a [Garmin Vivosmart 3](https://www.amazon.com/gp/product/B07BH3XN3T/ref=oh_aui_detailpage_o03_s00?ie=UTF8&psc=1), which was cheap had a decent sensor suite.
...
I originally got a Garmin device because it looked like I would have the best chance of being able to scoop off the raw data files of the device myself and bypass the ubiquitously opaque cloud services wearable vendors provide.
...
Garmin claims you should expect to wait two days (and up to 31 days, in the worst case), but the data's been prepared for me within a few minutes when I've requested it.

Garmin's raw data is bundled in a difficult-to-parse format called `.fit`, but that's a trade-off I'm willing to make in exchange for Garmin (albeit, as a compliance measure) curating, hosting, and making what comes off the device directly available to me.
...
That said, the `.fit` files are full of fun [undocumented ](https://www.thisisant.com/forum/viewthread/6374), [cryptic](https://forums.garmin.com/forum/on-the-trail/wrist-worn/fenix-5-5s/1375156-hrv-data-replaced-by-unknown-data-in-fit-files), and [nonstandard](https://forums.garmin.com/forum/into-sports/cycling/1245820-how-to-convert-timestamp) bits.
Plus, although the `.fit` data file standard's specifications are nominally made available, how exactly the engineering team behind the Vivosmart 3 *uses* that standard *definitely* is not.

However, on a cursory inspection it looks like I should be able to wring enough data out of these `.fit` files to tell an interesting story.
...
I just text the IFTTT messages like `#w XXX.X` to add an ntry
...
If you have other biometrics you want to track, it's easy to set up similar recipes that just use other hashtag codes.
...
I've already found it useful to dig up details I had otherwise forgotten to fill in the blanks on [anecdotes I wanted to re-tell](http://devosoft.org/reflections-on-my-first-semester-at-macdonald-middle-school/).
...
If I ever really wanted it, I might be able to GDPR it.

Email would probably be the most likely candidate for interesting personal analytics.
If you use google mail, you're [probably covered](https://support.google.com/accounts/answer/3024190?hl=en).

## Manual Data Management

Where to store my laptop log data?
Right now, I think I'll just plop it onto the two external hard drives I keep for my photos (one at work and the other at home).
...
I set myself a recurring reminder, so I'll at least know I'm dropping the ball if I lose interest in a month's time.
